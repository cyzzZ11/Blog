...
2. **用来检查访问权限**。例如：在进行 CORS 跨域资源共享时，对于复杂请求，就是使用 OPTIONS 方法发送嗅探请求，以判断是否有对指定资源的访问权限。
...
第一次握手：Client什么都不能确认，虽然他发了，但是他不知道对方收到了没有，不能确认自己的发送和接受都正常；Server 收到了SYN确认了自己接收正常，对方发送正常；
